...
import com.example.ISOFTBlockchain.block.HackedTransaction;
...
public class BlockchainServices {
    ...
    public List<Transaction> getTransactionsInPeriod(String accountNumber, Long start, Long end) {
        return accountService.getTransactionInPeriod(accountNumber, start, end);
    }
...
//        if (previousHash.equals(Constants.GENESIS_PREV_HASH)) return true;
//
//        String lastBlockHash = calculateLastBlockHash(block);
//    }
    ...
    public Account verifyAccounts() {
        ...
        for (int i = 0; i < dbAccounts.size(); i++) {
            ...
            if (!(dbAccount.toString().equals(memoryAccount.toString()))) {
                return new HackedAccount(dbAccount, memoryAccount);
            }
        }
        return null;
    }
    ...
    public Transaction verifyBlockchain() {
        ...
        for (int i = 0; i < dbBlockchain.size(); i++) {
            ...
            if (!(dbBlock.toString().equals(memoryBlock.toString()))) {
                return new HackedTransaction(dbBlock.getTransaction(), memoryBlock.getTransaction());
            }
        }
        return null;
    }
